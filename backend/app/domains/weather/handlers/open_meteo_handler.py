...
class OpenMeteoHandler(BaseHandler):
    def __init__(self, open_meteo_client: OpenMeteoClient=Depends()):
        super().__init__()
        self.open_meteo_client=open_meteo_client
    
    async def get_coordinates(self, city: str, country_code: str = None):
        try:
            data = await self.open_meteo_client.get_coordinates(city=city, country_code=country_code)
            if data:
                return data
    ...
    async def get_weather_info(self, latitude: float, longitude: float):
        try:
            data = await self.open_meteo_client.get_weather_info(latitude=latitude, longitude=longitude)
            if data:
                return data
        except Exception as e:
            logger.warning(f"OpenMeteo failed, trying next handler: {e}")
    ...
    async def get_air_quality(self, latitude: float, longitude: float):
        try:
            data = await self.open_meteo_client.get_air_quality(latitude=latitude, longitude=longitude)
            if data:
                return data
        except Exception as e:
            logger.warning(f"OpenMeteo failed, trying next handler: {e}")
    ...
    async def get_weather_forecast(self, latitude: float, longitude: float, timespan: str):
        try:
            data = await self.open_meteo_client.get_weather_forecast(latitude=latitude, longitude=longitude, timespan=timespan)
            if data:
                return data
        except Exception as e:
            logger.warning(f"OpenMeteo failed, trying next handler: {e}")
